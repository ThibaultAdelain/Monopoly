...
Propriete::Propriete(string n, int L, int prixA, int nb_maisons, int prix_maison, Case* suiv, Joueur* proprietaire):Case(n, suiv){
    this->nom = n;
    this->loyer = L;
...
void Propriete::arreterSur(Joueur* J){
        ...
        if (reponse == "oui" && (J->getSolde() - prixAchat) >= 0) {
            ...
            if (reponse=="oui") {
                ...
                if (J->getSolde() - nb_a_construire*prix_maison >= 0 && nb_a_construire + nb_maisons <=5) {
                    ...
                    cout << "\nVous avez " << nb_maisons << " maisons sur " << nom << endl;
